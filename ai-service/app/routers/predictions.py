...
# Historical time-to-sell: days between a property going live (created_at) and
# an offer being recorded on it (sale_files.offer_date, falling back to the sale
# file's creation date). Negative spans (bad data) are filtered out.
DELAY_SQL = """
...
def predict_delay(req: DelayRequest):
    ...
    Strategy mirrors the price estimate:
      - enough comparable sales (same city + category) -> linear regression on
        (area, price), since an over-priced or larger bien sells more slowly;
      - otherwise -> historical average delay (city, then category, then global).
    """
    comparables = query_df(
    ...
    predicted = max(predicted, 1.0)  # at least one day
    return {
